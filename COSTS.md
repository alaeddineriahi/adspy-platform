...
| 2026-07-03 | **Website Intel** added: 1 LLM call per analysis, ~4–13k input tokens (page text capped at 12k chars) + ~600 output. On Groq: $0. On Claude Sonnet: ~1–4¢ per analysis — the priciest single credit we sell, still fine at 1 credit. | Watch input size if we ever fetch multiple pages per analysis. |
| 2026-07-03 | **Brand deep-dive** added: each sweep makes up to 8 extra page-pinned Ad Library fetches (3 pages each) + R2 mirrors for ≤30 catalog ads/brand. No LLM. | $0 marginal (scraper session + R2 free tier). Real cost is FB-session load — capped per sweep + 24h/brand cooldown for exactly this reason. |
| 2026-07-04 | **Trend Radar** added: pure diffing of sweep state already computed (no extra scraping, no LLM, no polling). Events in Postgres, 14-day retention, capped per sweep. | $0 marginal — the highest value-to-cost feature in the product. |
| 2026-07-04 | **Product Dossier** added: 1 LLM call (~1–3k input tokens) + one ES more_like_this query per dossier; sourcing links are constructed URLs (no scraping). Sold at 2 credits. | Groq: $0 today. On Claude Sonnet: ~1–2¢/dossier vs 2 credits charged — margin-positive at any realistic volume. |
| 2026-07-06 | **TikTok ingestion** added: Creative Center Top Ads via a headless-browser fetcher (Node + Edge/Chrome, `backend/tools/`), 1 run/day across 6 markets (~1–2 min browser time), covers mirrored to R2. No LLM. | $0 marginal today. Production VPS needs Node + a chromium (~300MB disk); CPU negligible at daily cadence. R2 +~240 objects/day worst case — far inside free tier. |
| 2026-07-06 | **Dossier upgraded**: now up to 2 LLM calls (identity + landing-page funnel teardown, ~4–13k extra input) + up to 4 one-page Ad Library live-count probes (jittered, FB session). Still 2 credits. | Groq: $0. On Claude Sonnet: ~2–5¢/dossier worst case — re-check at Claude flip (§2); consider 3 credits then. Probe load on the FB session is negligible vs a sweep. |
| 2026-07-06 | **Index hygiene**: sweeps dedupe collated copies (one doc per creative per market) — 43% of existing docs were duplicates. | Halves ES churn and R2 mirror volume at equal coverage. |
| 2026-07-07 | **Brand Hunter** added: discovers viral/winning brands from live signals (TikTok top-ad brand names + rising scalers), resolves each name→page_id, and pulls its full Meta catalog via the shared deep-dive. Daily job, capped BRAND_HUNTER_PER_RUN=6 brands. No LLM. | $0 marginal (reuses scraper session + R2). Real cost is FB-session load: per run ≤6 resolves (1 search each) + ≤6 catalog dives — budgeted like the deep-dive, paced with jitter + a 48h/brand cooldown so it doesn't trip Meta's throttle. Web-research source left as a stub (would add a search-API cost only if enabled). |
