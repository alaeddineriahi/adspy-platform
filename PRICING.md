...
| 2026-07-03 | ✅ **Brand deep-dive** shipped: top scaling brands get their FULL live Ad Library catalog pulled by page_id → verified "N ads live now" counts, GetHookd-style "50+ ads live" filter in Brand Spy, snapshot trajectory ("+N live ads since tracking"), and each dive grows the catalog with the brand's best ads. Cross-market `countries` fix means pan-GCC ads match every market they run in. | This is the quality lever competitors sell as their headline ("brands with 50+ ads"). Candidate Pro gate: the 50+ filter + trajectory on paid tiers if free conversion needs a push. No price change yet. |
| 2026-07-04 | ✅ **Trend Radar** shipped: every sweep diffs the market and emits signals — new products scaling fast, steady→hot momentum flips, global winners arriving in MENA, brands jumping +N live ads, proven scalers shipping fresh creatives. Animated radar page + watch-a-brand alerts. **Free tier = teaser (3 signals + count of locked ones); Pro = everything.** Zero LLM cost. | This is the conversion feature: the locked-signals wall sells FOMO on *missing money*, far stronger than a credits wall. Radar alone justifies Pro at 79 TND. |
| 2026-07-04 | ✅ **Product Dossier** shipped: one click on any winning ad → product identified (LLM), honest margin math (extracted sell price − AliExpress supply band − ~18% COD fees), saturation score, **market-gap map** ("3 brands in SA, zero in MA — you're first"), competitor list, angles, risks, sourcing links, and Launch buttons into the ad-pack/media-buyer. **2 credits per dossier (1 when served from the 7-day server cache — no LLM cost).** | The Minea-style conversion feature, with a gap-map nobody else can produce for MENA. Biggest credit sink yet — watch usage; if dossiers dominate consumption, consider Pro 400→300 credits or price dossiers at 3. |
| 2026-07-06 | ✅ **TikTok Top Ads** shipped: second platform, 6 markets (US/GB/FR/SA/AE/EG), with REAL engagement (CTR, likes) Meta never exposes. Free to serve (no LLM). | Closes the #1 competitive gap vs Minea/PiPiADS/WinningHunter. Candidate Pro gate: TikTok platform filter (like the global-markets gate) if free→Pro conversion needs a push. No price change yet. |
| 2026-07-06 | ✅ **Swipe file completed**: save works everywhere, boards (create/organize/tabs), and the free-tier cap (10 saved ads) is now ENFORCED with an upgrade prompt — §2's table row finally exists in code. | First non-AI enforcement lever live. Watch conversion events from the cap toast. |
| 2026-07-06 | ✅ **Dossier upgraded, same 2 credits**: open markets now carry LIVE Ad Library counts ("TN · 0 live ✓" = verified first-mover claim) + automatic funnel teardown of the winner's own store (offer, price, COD, weaknesses to attack). | Deepens the moat feature at unchanged price; if Claude-era COGS bites (COSTS.md), dossiers move to 3 credits. |
| 2026-07-06 | ✅ **Search power filters** (momentum / age / scaling / est. spend) + honest brand-leaderboard scaling sums + creative launch timeline on brand pages. | Table-stakes parity with US tools (WinningHunter filter set, Foreplay Spyder timeline) — strengthens Pro's story, no price change. |
| 2026-07-06 | ✅ **Free tier ENFORCED (the paywall finally has a wall)**: Search & Brand Spy now require login (anonymous scraping killed) and the free plan is capped in code — top 25 search results, no power filters, top-5 brand leaderboard, all with in-product upgrade prompts. `/api/payments/plans` rewritten to advertise only what's actually enforced (the old `searches_per_day: 20` / `brand_spy: false` were fiction). | This is the conversion lever that was missing — free users now hit real walls that Pro removes. Expect this to be the biggest single driver of free→Pro once payments are live. |
| 2026-07-06 | ✅ **Catalog hardened**: internal scoring fields stripped from all public API responses (no more leaking how ranking works); `/docs`+OpenAPI closed in prod; DEBUG defaults off. Per-market keep cap 40→90 and a 6-market TikTok sweep deepened thin niches. | Protects the moat (the catalog can't be trivially cloned) and makes first-search results feel deep instead of empty. No price change. |
| 2026-07-07 | ✅ **Brand Hunter** shipped: on top of the keyword sweep, a daily discovery pass finds brands that are winning/going viral RIGHT NOW (TikTok Creative Center top-ad brands + rising scalers whose live-ad count jumped) and pulls their full Meta catalog — cross-platform discovery the keyword sweep structurally can't do. Data is always current (no LLM/stale lists). Free to serve. | Directly deepens the "we surface winners you can't find elsewhere" pitch and keeps the catalog fresh with proven brands. Candidate Pro gate later: a "just discovered / trending brands" feed. No price change. |
| 2026-07-07 | ✅ **"Just Discovered" feed** shipped (`/discovered`, top of the Discover nav): a live shelf of freshly full-catalog'd winning brands with WHY badges (🔥 Viral on TikTok / 📈 Scaling up / 💎 Top scaler), live-ad counts, and recency. Free sees a top-5 teaser; Pro sees the full feed. Also wired an OPTIONAL open-web research source for the hunter (Serper/Tavily/Brave behind SEARCH_API_KEY — see RESEARCH_API_SETUP.md). | Makes freshness visible (retention + "wow, new winners daily") and is a natural Pro gate. Web research is $0 until a key is added, and every provider's free tier covers our ~90 searches/mo. |
